...
**census fetches the US Census Bureau’s market-moving economic
indicators and population surveys and hands them back as tidy R
tables.**

The US Census Bureau is the official source of many of the government’s
hard economic numbers — how much shops are selling, how many new houses
are being started, how many new businesses are being formed, how much
the country trades with the rest of the world. Several of these figures
move markets the moment they are published. This package fetches those
official statistics directly from the Census Bureau and hands them back
as clean tables you can work with in R, and it also reaches the Bureau’s
population and demographic surveys (income, employment, and who lives
where). Looking up what data exists needs no sign-up; pulling the
numbers themselves needs a free key. It fetches and tidies the figures
faithfully and leaves the interpretation to you.
...
American Community Survey (ACS) demographic aggregates, plus keyless
dataset, variable, and geography discovery, built on the shared
...
Every EITS programme is queried the same way: a required `time`
predicate (a year, a month, or a `"from ... to ..."` range), optionally
narrowed by `category_code`, `data_type_code`, and `seasonally_adj`. The
result is long and tidy — one row per (period × category × data_type ×
adjustment).
...
The convenience wrappers pin the programme and document its release
cadence:
...
ACS gives income, employment, and demographics for every US geography,
as a wide table — one row per geography, one column per requested
variable. Estimate (`*E`) and margin-of-error (`*M`) columns are
numeric; annotation and code columns are character.
...
``` R
#>                      name b01001_001e b19013_001e b19013_001m b19013_001ea
#>                    <char>       <num>       <num>       <num>       <char>
#> 1: Alpha County, Testland      100000       65000         500         <NA>
#> 2:  Beta County, Testland      250000       82000        1200         <NA>
#> 3: Gamma County, Testland        5000          NA          NA         <NA>
#> 4: Delta County, Testland        1200  -666666666  -555555555         <NA>
#>     state county
#>    <char> <char>
#> 1:     90    001
#> 2:     90    003
...
Each survey year is its own dataset (there is no `time` predicate), so a
multi-year pull iterates the years and stacks them with a `year` column
— a year the Bureau never released (e.g. 1-year 2020) is skipped:
...
The wide columns are terse variable codes; look up their labels from the
keyless dictionary:
...
#>           name                                                   label
#>         <char>                                                  <char>
#> 1: B01001_001E                                        Estimate!!Total:
#> 2: B19013_001E Estimate!!Median household income in the past 12 months
```

The whole grammar validates against the geography `requires` chains
before spending a keyed call: a wildcard child (`county:*`) may omit its
parents, but a specific one (`county:037`) must supply them — mirroring
the API exactly.

## Multi-year backfill

[`census_backfill_series()`](https://dereckscompany.github.io/census/reference/census_backfill_series.md)
pages the `time` predicate across a range of years, deduplicates, and
returns one tidy series:
...
This package has no vignettes yet; the sections above are the whole of
its worked documentation.

Release history is in
[`NEWS.md`](https://dereckscompany.github.io/census/NEWS.md).

## Citation

Cite as: Mezquita, D. (2026). census: API Wrapper to the US Census
Bureau Data API. R package version 0.2.3.
